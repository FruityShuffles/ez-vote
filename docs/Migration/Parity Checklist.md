...
- [ ] 🔴 **INV-02** — A "pending invitees" affordance lists invited-but-not-yet-voted voters, visible to all viewers, and **refreshes** as people vote. _Source:_ #57, #84. _Verify:_ invitee votes while sheet is open → drops off pending list. _(M12 wired the add-time half: `useAddVoterToElection` invalidates `pending-invitees`, and `PendingInviteesRow` renders the list. The live refresh-as-votes-arrive — the Flutter 10s `results.updated_at` poll — is **M15** with the rest of realtime, matching how `ElectionDetail` already deferred its poll.)_
- [ ] 🔴 **INV-03** — The submitted-voters list refreshes to include a new voter who submits while you are already on the screen (don't snapshot it once at mount). _Source:_ #84. _Verify:_ open detail, have another user vote → their name appears. _(Deferred to **M15** with the live poll — same reason as INV-02.)_
