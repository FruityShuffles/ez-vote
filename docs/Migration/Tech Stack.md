...
- **It mirrors the current deployment.** Flutter ships today as a static build on **Cloudflare Pages**. A Vite SPA deploys the identical way (static assets → Pages CDN), so M5/M17 inherit the existing pipeline, `_redirects` SPA-routing trick, and `--dart-define`-style build-time env injection — rather than standing up new infrastructure.
...
- **M6 (auth):** Supabase client + TanStack Query; React Router guards reproduce the `redirect=` threading in `router.dart` (AUTH-01).
- **M7 (design system):** Tailwind tokens mapped from the Flutter theme; shared components built on Radix/shadcn with a11y verified.
