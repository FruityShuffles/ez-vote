// Pure tabulation logic shared by compute-results (and the future
// simulate-counterfactual function). No Deno or network imports — this module
// must stay importable from any TypeScript runtime (Deno, Node, browser).
...
// ---- Orchestrator ----
// Single entry point owning algorithm dispatch and the FPTP irv-fallback
// wiring, so every caller (compute-results, simulate-counterfactual) tabulates
// identically. Unknown algorithm names produce an empty result_data object,
// matching the original switch's default behavior.
export function tabulate(
  ...
  for (const algo of algorithms) {
    ...
    results.push({ algorithm: algo, result_data: resultData });
  }
  ...
  if (includeFptp) {
    ...
    results.push({
      algorithm: "fptp",
      result_data: computeFPTP(candidates, ballots, hasIrv),
    });
  }
